...
export async function POST(req: NextRequest) {
  ...
  const userMessage = buildAnalyseUserMessage(
    jobAd,
    companyName,
    companyDesc,
    iterationNote,
  ...
  try {
    ...
    raw = text;
    console.log(
      `[analyse] model=${model} raw (first 800 chars):`,
      raw.slice(0, 800),
    );
  } catch (err) {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "We've hit our AI usage limit. Please try again in a few minutes.",
        },
        { status: 429 },
      );
    }
  ...
  try {
    ...
    return fail500(
      "Analysis failed, please try again",
      `JSON.parse failed: ${msg} — raw: ${raw.slice(0, 200)}`,
    );
  }
  ...
  try {
    ...
    return fail500(
      "Analysis failed, please try again",
      `validateDiagnosis failed: ${msg} — parsed: ${snippet}`,
    );
  }
  ...
  if (!diagnosis.isLegal) {
    return NextResponse.json(
      {
        error: "We can't analyse this ad.",
        reason: diagnosis.illegalReason ?? null,
      },
      { status: 422 },
    );
  }
